...
. $PSScriptRoot\lib\text-helpers.ps1;
. $PSScriptRoot\lib\git-helpers.ps1;
. $PSScriptRoot\lib\github-helpers.ps1;
. $PSScriptRoot\lib\7zip-helpers.ps1;
. $PSScriptRoot\lib\deployment-helpers.ps1;
...
try {
    ...
    if ($isGitIntegrated) {
        ...
        if ($null -eq $hash) {
            $hash = (Get-RandomHexString -Bits 128).ToLower().Substring(0, 7);
        }
    ...
    $versioningTag = "$($date.Year).
                    $(Get-ParsedNumber -num $date.Month).
                    $(Get-ParsedNumber -num $date.Day)-
                    $(Get-ParsedNumber -num $date.Hour).
                    $(Get-ParsedNumber -num $date.Minute).
                    $(Get-ParsedNumber -num $date.Second)_
                    $($branch)-$($random)_
                    $($hash)" -replace '\s+', '';
    ...
    & 7z a -r -bb3 -y -tzip $sourceArchive "$componentDir\*.*" "-x!*.config" "-x!appsettings.json" "-x!appsettings.*.json"
    & 7z a -r -bb3 -y -tzip $configArchive "$componentDir\*.config" "$componentDir\appsettings.json" "$componentDir\appsettings.*.json"
    ...
    if ($checkForExistingSourceArchive) {
        ...
        if (Test-Hash -NewLocation $sourceArchive -ExistingLocation $existingSourceArchive) {
    ...
    if ($checkForExistingConfigArchive) {
        if (Test-Hash -NewLocation $configArchive -ExistingLocation $existingConfigArchive) {
