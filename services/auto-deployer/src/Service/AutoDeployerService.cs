...
internal static class AutoDeployerService
{
    ...
    private static bool CachedVersionExistsRemotely(out bool wasRatelimited, out TimeSpan? retryAfter)
    {
        ...
        catch (ApiException ex) when (ex.StatusCode is HttpStatusCode.NotFound or (HttpStatusCode)429 or HttpStatusCode.Forbidden)
        {
            ...
            if (!int.TryParse(ex.HttpResponse.Headers
                .FirstOrDefault(h => h.Key == "X-RateLimit-Remaining")
                .Value, out var rateLimitRemaining)) return false;

            wasRatelimited = true;

            if (!int.TryParse(ex.HttpResponse.Headers
                .FirstOrDefault(h => h.Key == "X-RateLimit-Reset")
                .Value, out var rateLimitReset)) return false;

            var rateLimitResetDateTime = DateTimeOffset.FromUnixTimeSeconds(rateLimitReset);
    ...
    private static DateTime GetDateTimeFromVersionString(this string str)
    {
        ...
        var groups = Regex.Match(str, DeploymentIdRegex).Groups;
        if (groups == null)
    ...
    {
        _versioningRegSubKey = AutoDeployerSettings.Singleton.VersioningRegistrySubKey;
        _versioningRegVersionKeyName = AutoDeployerSettings.Singleton.VersioningRegistryVersionKeyName;
        _deploymentPath = AutoDeployerSettings.Singleton.DeploymentPath;
        _githubOrgOrAccountName = AutoDeployerSettings.Singleton.GithubAccountOrOrganizationName;
        _githubRepositoryName = AutoDeployerSettings.Singleton.GithubRepositoryName;
        _primaryDeploymentExecutable = AutoDeployerSettings.Singleton.DeploymentPrimaryExecutableName;
        _pollingInterval = AutoDeployerSettings.Singleton.PollingInterval;
        _skippedVersionsInvalidationInterval = AutoDeployerSettings.Singleton.SkippedVersionInvalidationInterval;
        _deploymentAppName = AutoDeployerSettings.Singleton.DeploymentAppName?.ToLower();
        ...
        _githubToken = AutoDeployerSettings.Singleton.GithubToken.FromEnvironmentExpression<string>();
        var gheUrl = AutoDeployerSettings.Singleton.GithubEnterpriseUrl;
        ...
        if (!BacktraceSettings.Singleton.BacktraceUrl.IsNullOrEmpty())
        {
            BacktraceCredentials backtraceCredentials = null;

            if (!BacktraceSettings.Singleton.BacktraceToken.IsNullOrEmpty())
        ...
        if (AutoDeployerSettings.Singleton.OnlyDeployPreRelease)
    ...
    private static bool GetProcessByName(string processName, out Process p)
    {
        p = null;
...
#if GRID_BOT
                    if (GetProcessByName(_primaryDeploymentExecutable.ToLower().Replace(".exe", ""), out _))
                    {
...
SLEEP:
    ...
    {
        for (int i = 1; i <= AutoDeployerSettings.Singleton.WatchDogMaxAttempts; i++)
        {
            StartNewProcess(primaryExe, workingDirectory);

            Thread.Sleep(AutoDeployerSettings.Singleton.WatchDogWaitTime);

            if (GetProcessByName(_primaryDeploymentExecutable.ToLower().Replace(".exe", ""), out _)) return;

            _logger.Warning(
                "Process '{0}' was not open after {1}, number of reattempts: {2}",
                primaryExe,
                AutoDeployerSettings.Singleton.WatchDogWaitTime,
                i - 1
            );
        }

        ReportToBacktrace("Process '{0}' did not open after {1} attempts, send this to Backtrace!", primaryExe, AutoDeployerSettings.Singleton.WatchDogMaxAttempts);
    }
    ...
    private static void CollectLogsAndReportToBacktrace()
    {
        PercentageInvoker.Singleton.InvokeAction(
            ...
            {
#if DEBUG
                var prefix = "dev_" + AutoDeployerSettings.Singleton.EnvironmentLoggerName;
#else
                var prefix = AutoDeployerSettings.Singleton.EnvironmentLoggerName;
#endif
    ...
    public static string GetProcessOwnerByProcessId(int processId)
    {
        ...
        foreach (var argList in from ManagementObject obj in searcher.Get()
                let argList = new string[] { string.Empty, string.Empty }
                let returnVal = Convert.ToInt32(obj.InvokeMethod("GetOwner", argList))
                where returnVal == 0
                select argList)
        {
            // return DOMAIN\user
    ...
    private static bool ProcessIsElavatedByHandle(IntPtr hProcess)
    {
        ...
        try
        {
            ...
            var identity = new WindowsIdentity(processHandle.DangerousGetHandle());
    ...
    {
        if (!GetProcessByName(name.ToLower().Replace(".exe", ""), out var pr))
        {
            _logger.Warning("The process '{0}' is not running, ignoring...", name);
            return;
        }

        if (!_isAdministrator && ProcessIsElevatedByProcessId(pr.Id))
        {
            ...
            while (true)
            {
                ...
                if (latestRelease.Prerelease)
                {
                    if (!latestRelease.Name.IsMatch(ShouldDeployPreReleaseSearchString, RegexOptions.Compiled | RegexOptions.IgnoreCase) &&
                        !AutoDeployerSettings.Singleton.OnlyDeployPreRelease)
                    {
                ...
                var groups = Regex.Match(latestRelease.Name, DeploymentIdRegex, RegexOptions.Compiled | RegexOptions.IgnoreCase).Groups;
    ...
    private static void DetermineLatestVersion()
    {
        ...
        var directoryMatches = (from f in Directory.EnumerateDirectories(_deploymentPath)
                                let match = Regex.Match(f, DeploymentIdRegex, RegexOptions.Compiled)
                                where match.Success
                                select match).ToList();
        ...
        var sortedDirectories = from m in directoryMatches
                                let groups = m.Groups
                                let year = int.Parse(groups["year"].Value)
                                let month = int.Parse(groups["month"].Value)
                                let day = int.Parse(groups["day"].Value)
                                let hour = int.Parse(groups["hour"].Value)
                                let minute = int.Parse(groups["minute"].Value)
                                let second = int.Parse(groups["second"].Value)
    ...
    private static void SetupDeploymentPath()
    {
        ...
        if (!AutoDeployerSettings.Singleton.CreateDeploymentPathIfNotExists)
        {
