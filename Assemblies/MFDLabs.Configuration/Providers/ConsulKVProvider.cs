...
namespace MFDLabs.Configuration.Providers
{
    public class ConsulKVProvider : SettingsProvider
    {
        static ConsulKVProvider()
        {
            ...
            _configurationSection = ConfigurationManager.GetSection("mfdlabsConsulConfiguration") as ConsulKVProviderConfigurationSection;
            if (_configurationSection != null)
            {
                var configuration = GetGroupConfigurationElement();
                var address = _address = configuration.Address;
                _configurationClient = new ConsulKVConfigurationClient(address, configuration.ACLToken);
                ...
                _providers = new ConcurrentDictionary<string, ConsulKVProvider>();
        ...
        public override string Description
        {
            get
            {
                return "A consul-backed SettingsProvider that synchronizes assembly settings from a common consul kv repository.";
            }
        }
        ...
        private void UpdateConnectionStringsFromConsulKV(Dictionary<string, SettingsPropertyValue> settingsProperties, int maxAttempts, DateTime lastModification)
        {
        ...
        private void UpdateSettingsFromConsulKV(Dictionary<string, SettingsPropertyValue> settingsProperties, int maxAttempts, DateTime lastModification)
        {
        ...
        public override SettingsPropertyValueCollection GetPropertyValues(SettingsContext context, SettingsPropertyCollection collection)
        {
            ...
            if (_hasSettings)
            {
                UpdateSettingsFromConsulKV(settingsProperties, maxAttempts, DateTime.MinValue);
            }
            if (_hasConnectionStrings)
            {
                UpdateConnectionStringsFromConsulKV(settingsProperties, maxAttempts, DateTime.MinValue);
            }
            return propertyValues;
        }
...
#pragma warning disable IDE0052 // Remove unread private members
        ...
        private static readonly ConsulKVProviderConfigurationSection _configurationSection;
        private static readonly ConsulKVConfigurationClient _configurationClient;
        private static readonly string _address;
        private static readonly SelfDisposingTimer _timer;
        private static readonly ConcurrentDictionary<string, ConsulKVProvider> _providers;
