	...
	function Capsule:__index(k)
		if k:lower() == "getservice" then
			return function(...)
				...
				if service == game:GetService("HttpService") or service == game:GetService("HttpRbxApiService") then
					last = service
				end
				return service
			end
		end
		if last == game or last == game:GetService("HttpService") or last == game:GetService("HttpRbxApiService") then
			if 
				k:lower() == "postasyncfullurl" or 
				k:lower() == "requestasyncfullurl" or 
				k:lower() == "getasyncfullurl" or
				k:lower() == "postasync" or
				k:lower() == "requestasync" or
				k:lower() == "getasync" or 
				k:lower() == "requestasync" or
				k:lower() == "httppostasync" or
				k:lower() == "httppost" or
				k:lower() == "httpgetasync" or
				k:lower() == "httpget" or
				k:lower() == "requestinternal" then
					return function (...) error(string.format("The method by the name of '%s' is disabled.", k)) end 
				end
		elseif typeof(last) == "Instance" then
			if last:IsA("NetworkClient") or last:IsA("NetworkMarker") or last:IsA("NetworkPeer") or last:IsA("NetworkReplicator") or last:IsA("NetworkServer") or last:IsA("NetworkSettings") then
				return nil
			end
		end
		last = self[k]
