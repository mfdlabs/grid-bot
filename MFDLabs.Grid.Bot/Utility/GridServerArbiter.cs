...
namespace MFDLabs.Grid.Bot.Utility
{
    ...
    public sealed class GridServerArbiter : SingletonBase<GridServerArbiter>
    {
            ...
            bool startUp = true)
        {
            ...
            for (var i = 0; i < count; i++) instances.Add(QueueUpArbiteredInstance(null, maxAttemptsToHitGridServer, hostName, startUp));
            return instances;
        }
        ...
        {
            lock (_instances)
                return (from instance in _instances where instance.Name == name && instance.Endpoint.Address.Uri.Host == hostName && instance.Persistent select instance).FirstOrDefault();
        }
        ...
        private void InvokeMethod(string method, string name, int maxAttemptsToHitGridServer, string hostName, bool isPoolable, params object[] args)
            => InvokeMethod<object>(method, name, maxAttemptsToHitGridServer, hostName, isPoolable, args);
        private T InvokeMethod<T>(string method, string name, int maxAttemptsToHitGridServer, string hostName, bool isPoolable, params object[] args)
        {
        ...
        private async Task InvokeMethodAsync(string method, string name, int maxAttemptsToHitGridServer, string hostName, bool isPoolable, params object[] args)
            => await InvokeMethodAsync<object>(method, name, maxAttemptsToHitGridServer, hostName, isPoolable, args);
        private async Task<T> InvokeMethodAsync<T>(string method, string name, int maxAttemptsToHitGridServer, string hostName, bool isPoolable, params object[] args)
        {
            _perfmon.TotalInvocations.Increment();
        ...
        public class GridServerInstance : ComputeCloudServiceSoapClient, IDisposable
        {
            ...
            private GridServerInstance(EndpointAddress remoteAddress, string name, bool openProcessNow, int maxAttemptsToHitGridServer = 5, bool persistent = false, bool poolable = true, bool openNowInNewThread = false)
            ...
            private void LockAndTryOpen()
            {
                ...
                if (!TryOpen()) 
                    throw new ApplicationException("Unable to open grid server instance.");
            }
