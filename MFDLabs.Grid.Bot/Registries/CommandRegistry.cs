...
namespace MFDLabs.Grid.Bot.Registries
{
    public static class CommandRegistry
    {
        ...
        private static readonly ICollection<CommandCircuitBreakerWrapper> CircuitBreakerWrappers =
            new List<CommandCircuitBreakerWrapper>();
        ...
        private static string GetDefaultCommandNamespace() => $"{typeof(Program).Namespace}.Commands";

#if WE_LOVE_EM_SLASH_COMMANDS

        private static string GetSlashCommandNamespace() => $"{typeof(Program).Namespace}.SlashCommands";
        ...
        private static void ExecuteCommandInNewThread(string alias, string[] messageContent, SocketMessage message, Stopwatch sw, IStateSpecificCommandHandler command)
        {
            ...
            ThreadPool.QueueUserWorkItem(async _ =>
            {
                try
                {
                    InstrumentationPerfmon.NewThreadCommandsThatPassedChecks.Increment();
                    // We do not expect a result here.
                    await GetWrapperByCommand(command).ExecuteAsync(messageContent,
                        message,
                        alias);
                    InstrumentationPerfmon.SucceededCommandsPerSecond.Increment();
                    Counters.RequestSucceededCountN++;
        ...
        private static async Task HandleException(Exception ex, string alias, SocketMessage message)
        {
            ...
            switch (ex)
            {
                ...
                case FaultException fault:
                {
                    ...
                    if (fault.Message.Length > EmbedBuilder.MaxDescriptionLength)
                    {
                        await message.Channel.SendFileAsync(new MemoryStream(Encoding.UTF8.GetBytes(fault.Message)),
                            "fault.txt",
        ...
        private class CountersData
        {
            internal int RequestCountN;
            internal int RequestFailedCountN;
            internal int RequestSucceededCountN;
        }
