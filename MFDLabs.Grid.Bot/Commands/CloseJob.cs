...
namespace MFDLabs.Grid.Bot.Commands
{
    internal class CloseJob : IStateSpecificCommandHandler
    {
        ...
        public string CommandDescription => $"Attempts to close a grid server job via the SoapUtility\nLayout:" +
                                            $"{MFDLabs.Grid.Bot.Properties.Settings.Default.Prefix}closejob jobID.";
        public string[] CommandAliases => new[] { "cj", "closejob" };
        ...
        public async Task Invoke(string[] messageContentArray, SocketMessage message, string originalCommand)
        {
            ...
            if (jobId == default)
            {
                await message.ReplyAsync($"Missing required parameter 'jobId', the layout is: " +
                                         $"{MFDLabs.Grid.Bot.Properties.Settings.Default.Prefix}{originalCommand} jobID");
                return;
            }
