                    ...
                    if (!global::Grid.Bot.Properties.Settings.Default.IsEnabled)
                    {
                        if (!optionalMessage.IsNullOrEmpty() && optionalMessage != global::Grid.Bot.Properties.Settings.Default.ReasonForDying)
                        {
                            await message.ReplyAsync("The maintenance status is already enabled, and it appears you have a different message, " +
                                                    ...
                                                    $"'{(global::Grid.Bot.Properties.Settings.Default.Prefix)}{originalCommand} update {optionalMessage}'");
                            return;
                        }
                ...
                case "disable":
                    if (global::Grid.Bot.Properties.Settings.Default.IsEnabled)
                    {
                        await message.ReplyAsync("The maintenance status is not enabled! " +
                                                 ...
                                                 $"'{(global::Grid.Bot.Properties.Settings.Default.Prefix)}{originalCommand} enable optionalMessage?'");
                        return;
                    }
