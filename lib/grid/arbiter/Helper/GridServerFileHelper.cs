...
public class GridServerFileHelper : IGridServerFileHelper
{
    ...
    public GridServerFileHelper(ISettings settings)
    {
    ...
    public string GetGridServerPath(bool throwIfNoGridServer = true)
    {
        ...
        if (throwIfNoGridServer)
            throw new ApplicationException(Resources.GridServerNotCorrectlyInstalled);
    ...
    )
    {
        ...
        if (!File.Exists(fullPath))
            throw new ApplicationException(string.Format(Resources.CouldNotFindGridServerLuaScript, scriptName, prefix));
