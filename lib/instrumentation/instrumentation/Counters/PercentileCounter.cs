...
namespace MFDLabs.Instrumentation
{
    internal class PercentileCounter : IPercentileCounter
    {
        ...
        internal IReadOnlyCollection<KeyValuePair<CounterKey, double>> Flush()
        {
            ...
            if (exchangedBag.IsEmpty) return Array.Empty<KeyValuePair<CounterKey, double>>();
        ...
        internal IReadOnlyCollection<KeyValuePair<CounterKey, double>> Get()
        {
            ...
            if (percentileCounters.Count == 0) return Array.Empty<KeyValuePair<CounterKey, double>>();
        ...
        {
            if (sortedValues.Count == 0) throw new ArgumentException(string.Format("{0} count must be > 0", "sortedValues"), nameof(sortedValues));
            ...
            if (index + 1 < sortedValues.Count) return sortedValues[index] * (1.0 - switchBackPercentile) + sortedValues[index + 1] * switchBackPercentile;
            return sortedValues[index];
        }
        internal static void ValidatePercentiles(byte[] percentiles)
        {
            if (percentiles == null || percentiles.Length == 0) throw new ArgumentException("Percentiles cannot be null or empty", nameof(percentiles));
            foreach (byte percentile in percentiles)
                if (percentile < 1 || percentile > 99)
                    throw new ArgumentException("Percentiles cannot be < 1 or > 99.  The value was " + percentile, nameof(percentiles));
        }
        internal IReadOnlyCollection<KeyValuePair<CounterKey, double>> ComputePercentiles(IList<double> sortedValues)
        {
            ...
            foreach (var percentileKey in _CounterKeysByPercentile) 
                percentileKeys.Add(new KeyValuePair<CounterKey, double>(percentileKey.Value, GetPercentile(sortedValues, percentileKey.Key / 100)));
